...
如果配置了 `output_file`，统计数据会**每分钟覆盖写入**到指定文件，方便随时查看最新数据。
...
```
更新时间: 2025-12-03 09:18:48

=== IP 流量统计（TOP 10）===
排名 IP 地址                                      上传         下载         总流量      连接数
----------------------------------------------------------------------------------------------------
1    192.168.1.100                              1.25 GB      3.45 GB      4.70 GB      1523
2    192.168.1.101                              800.50 MB    2.10 GB      2.90 GB      856
3    192.168.1.102                              500.20 MB    1.50 GB      2.00 GB      432
----------------------------------------------------------------------------------------------------
当前跟踪 IP 数量: 45
```
...
- ✅ **覆盖写入**：每次更新都覆盖整个文件，只保留最新数据
- ✅ **时间戳**：文件开头显示最后更新时间
- ✅ **易于查看**：可以用 `cat`、`tail`、`less` 等命令查看
- ✅ **无日志干扰**：纯净的统计数据，不包含日志级别等信息
- ✅ **实时监控**：使用 `watch -n 1 cat ip_traffic_stats.txt` 实时查看

#### 实时查看命令

```bash
# 查看最新统计
cat ip_traffic_stats.txt

# 实时监控（每秒刷新）
watch -n 1 cat ip_traffic_stats.txt
